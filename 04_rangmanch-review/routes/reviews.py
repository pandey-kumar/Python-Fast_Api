...
def get_reviews(
    play_name:str|None =Query(None,description="Filter by play_name"),
...
def get_average(play_name:str,session:Session=Depends(get_session)):
   result=session.exec(
...
def get_review(review_id:int ,session:Session=Depends(get_session)):
   ...
   if not review:
      raise HTTPException(status_code=404,detail=f"No review found for review_id {review_id}")
...
def update_review(review_id:int,update:ReviewUpdate,session:Session=Depends(get_session)):
   review=session.get(Review,review_id)
   if not review:
      raise HTTPException(status_code=404,detail=f"Review not present for Review id : {review_id}")
...
def delete_review(review_id:int,session:Session=Depends(get_session)):
   review=session.get(Review,review_id)
   if not review:
      raise HTTPException(status_code=404,detail=f"Review not found for review_id:{review_id}")
